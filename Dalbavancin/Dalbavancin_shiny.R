...
dalbaRegisterAddRemove <- function(input, output, session,
  ...
  observeEvent(input[[input_add]], {
    # optional per-covariate check: refuse to record an implausible value
    if (!is.null(validate) && !isTRUE(validate(input[[input_value]]))) {
      return(NULL)
    }
    new_entry <- tibble(
...
run_map_estimation <- function(patient_data_for_map2, model_list, free_drug) {
  ...
  # Individual PK parameters to harvest from the solved MAP model (whatever
  # subset each model defines). The rxode2 output contains them at every time
  # point, already combining typical value x covariate effects x exp(eta_MAP).
  # All models in models.R use this common naming: CL, V1..V3, Q (2-cpt) or
  # Q2/Q3 (3-cpt).
  param_candidates <- c("CL", "V1", "V2", "V3", "Q", "Q2", "Q3")
  ...
  for (i in seq_along(model_list)) {
    tryCatch({
      ...
      # Individual parameters over the whole profile. The MAP eta is constant,
      # but the covariate part is not: with serial creatinines (or weights) CL
      # and the volumes change over time, so a single snapshot would be wrong.
      # Consecutive time points sharing the same parameter set are collapsed
      # into one "epoch", keyed by the time the values start to apply.
      present <- intersect(param_candidates, names(model_dt))
      if (length(present) > 0) {
        pv   <- model_dt[, present, with = FALSE]
        key  <- do.call(paste, c(lapply(pv, function(x) signif(x, 6)), list(sep = "|")))
        keep <- c(TRUE, key[-1] != key[-length(key)])

        p_out[[i]] <- data.table::melt(
          model_dt[keep, c("time", present), with = FALSE],
          id.vars         = "time",
  ...
  # Bayesian model-averaging weights, derived once per model from its MAP OFV:
  #   LL_i = -OFV_i / 2      w_i = exp(LL_i) / sum_j exp(LL_j)
  # The weights are computed on the difference from the best OFV. This is
  # algebraically identical (the common factor cancels in the normalisation)
  # but avoids exp(-OFV/2) underflowing to 0 -- which happens above OFV ~1400
  # and would make every weight NaN. Aggregating per model_name also keeps two
  # models with an identical OFV from being collapsed into one.
  w_tbl <- unique(m_dt_out[, .(model_name, ofv)])
  w_tbl[, LL     := exp(-0.5 * (ofv - min(ofv)))]
  w_tbl[, weight := LL / sum(LL)]
  m_dt_out[w_tbl, on = "model_name", `:=`(LL = i.LL, weight = i.weight)]
  ...
  list(
    m_dt_out = m_dt_out,
    params   = data.table::as.data.table(do.call(rbind, p_out))
  )
}


# Individual MAP parameters of one model, formatted for display: one row per
# covariate epoch (dated), one column per parameter, 3 significant digits.
# When the covariates never change the whole profile collapses to a single
# row and the date column is dropped.
format_map_params <- function(params, which_model, first_dose_time) {
  label_map <- c(
    CL = "CL (L/h)",
    V1 = "V1 (L)",   V2 = "V2 (L)",   V3 = "V3 (L)",
    Q  = "Q (L/h)",  Q2 = "Q2 (L/h)", Q3 = "Q3 (L/h)"
  )
  ordered_labels <- unique(unname(label_map))

  d <- params %>%
    dplyr::filter(model_name == which_model) %>%
    dplyr::mutate(
      Parameter = factor(unname(label_map[parameter]), levels = ordered_labels),
  ...
  wide <- d %>%
    dplyr::mutate(
      From = format(first_dose_time + as.difftime(time, units = "hours"), "%d/%m/%Y")
    ) %>%
    dplyr::select(time, From, Parameter, value) %>%
    tidyr::pivot_wider(names_from = Parameter, values_from = value) %>%
    dplyr::arrange(time)

  if (nrow(wide) == 1) {
    wide %>% dplyr::select(-time, -From)   # covariates constant: no dating needed
  } else {
    wide %>% dplyr::select(-time)          # one row per covariate epoch
  }
}
...
# Contiguous "Below"-target days -> calendar intervals, used to shade the plots.
# AUC24 evaluated at time t covers the interval (t-24, t] h; consecutive Below
# days (24 h apart) are merged into a single band. The t = 0 row is skipped
# (its AUC24 is zero by construction, not a real sub-target interval).
compute_below_bands <- function(auc_data_plot, first_dose_time, max_time = NULL) {
  below <- auc_data_plot %>%
    filter(time > 0, above_target == "Below")
  if (!is.null(max_time)) below <- below %>% filter(time <= max_time)
  if (nrow(below) == 0) return(NULL)

  below %>%
    arrange(time) %>%
    mutate(new_run = c(TRUE, diff(time) > 24), run = cumsum(new_run)) %>%
    group_by(run) %>%
    summarise(start = min(time) - 24, end = max(time), .groups = "drop") %>%
    mutate(
      xstart = first_dose_time + as.difftime(start, units = "hours"),
      xend   = first_dose_time + as.difftime(end,   units = "hours")
    )
}


# Convert the band intervals into plotly rect shapes (red, semi-transparent,
# drawn below the traces, spanning the full plot height).
below_band_shapes <- function(bands) {
  if (is.null(bands) || nrow(bands) == 0) return(list())
  lapply(seq_len(nrow(bands)), function(i) {
    list(
      type = "rect", xref = "x", yref = "paper",
      x0 = format(bands$xstart[i], "%Y-%m-%d %H:%M:%S"),
      x1 = format(bands$xend[i],   "%Y-%m-%d %H:%M:%S"),
      y0 = 0, y1 = 1,
...
build_moa_plot <- function(data_prova, data_plot_dv, p_weights, below_shapes = list()) {
  plot_moa <- plot_ly() %>%
    add_lines(
      data      = data_prova,
      x         = ~Time,
    ...
    layout(
      ...
      showlegend = TRUE,
      legend     = list(orientation = "h", x = 0.5, y = -0.3, xanchor = "center", yanchor = "bottom"),
      shapes     = below_shapes  # red bands: days with fAUC24/MIC below target
    )
  ...
  subplot(
    ...
    layout(
      showlegend = TRUE,
      legend     = list(orientation = "h", x = 0.5, y = -0.3, xanchor = "center", yanchor = "bottom", hovermode = "x unified")
    )
}


build_single_model_plot <- function(data_plot, data_plot_map, data_plot_dv, best_model, below_shapes = list()) {
  plot_ly() %>%
    add_lines(
      data      = data_plot,
...
server <- function(input, output, session) {
  ...
  # Albumin is expected in g/dL (Carrothers2020 centres on 3.7, Lodise2026 on 2.8).
  # A value above 6 almost certainly means the user entered g/L.
  observe({
    hideFeedback("albumin")
    if (isTruthy(input$albumin)) {
      if (input$albumin > 6) {
        showFeedbackDanger("albumin", "Implausible in g/dL - did you enter g/L? Divide by 10.")
      } else if (input$albumin < 1) {
        showFeedbackWarning("albumin", "Below 1 g/dL - please check the value.")
      }
    }
  })
  ...
  covariate_handlers <- list(
    ...
    list(add = "addAlbumin",   remove = "removeAlbumin",   time = "albumin_time",   value = "albumin",   var = "albumines",   col = "ALBUMIN",
         validate = function(x) isTruthy(x) && x <= 6)
  )
  ...
  run_calculation <- function() {
    ...
    map_result <- run_map_estimation(patient_data_for_map2, model_list, free_drug)
    m_dt_out   <- map_result$m_dt_out
    ...
    # Below-target red bands ---------------------------------------------------
    # MoA plot: days where the model-averaged fAUC24/MIC is below target.
    shapes_moa <- below_band_shapes(compute_below_bands(
      auc_result$auc_data_plot, d_info$first_dose_time,
      max_time = max(data_prova$time)
    ))

    # Single-model plot: same computation on the best model's own unbound AUC.
    auc_result_best <- compute_auc_targets(
      result %>% select(time, Cc, AUC = AUC_free) %>% distinct(),
      target_mic, target_auc, d_info$last_time_relative
    )
    shapes_best <- below_band_shapes(compute_below_bands(
      auc_result_best$auc_data_plot, d_info$first_dose_time,
      max_time = max(data_plot$time)
    ))

    # Build plots -------------------------------------------------------------
    p_weights    <- build_weight_plot(metrics_long)
    combined_plot <- build_moa_plot(data_prova, data_plot_dv, p_weights, below_shapes = shapes_moa)
    plot_dalba   <- build_single_model_plot(data_plot, data_plot_map, data_plot_dv, best_model, below_shapes = shapes_best)
    ...
    # Individual MAP PK parameters of the best model --------------------------
    params_best_tb <- format_map_params(map_params, best_model, d_info$first_dose_time)
    ...
    output$box_parameters <- renderUI({
      box(
        ...
        div(
          style = "padding: 20px;",
          h4("Covariates"),
          DTOutput("combinedTable"),
          hr(),
          h4(sprintf("Individual PK parameters (%s)", best_model)),
          p(
            if (nrow(params_best_tb) > 1) {
              "MAP Bayesian estimates conditioned on the patient's TDM. The covariates changed during follow-up, so one row is shown per period, dated from when those values start to apply."
            } else {
              "MAP Bayesian estimates conditioned on the patient's TDM. The covariates did not change during follow-up, so these values apply to the whole profile."
            },
            style = "color: #666; font-size: 13px;"
          ),
          DTOutput("mapParamsTable")
        )
      )
    })
    ...
    output$mapParamsTable <- renderDT({
      opts <- if (nrow(params_best_tb) > 10) {
        list(dom = "tp", pageLength = 10)   # many covariate epochs: paginate
      } else {
        list(dom = "t")
      }
      datatable(params_best_tb, options = opts, rownames = FALSE)
    })
