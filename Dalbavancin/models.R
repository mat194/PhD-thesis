Dalbavancin <- list(
  Gregoire2025 = list(
    ...
    omega = lotri::lotri({ # Table 3: BSV reported as CV% (CL 19.85, V1 17.21);
      ETA_CL + ETA_V1 ~    
        c(log((19.85 / 100)^2 + 1),
          0, log((17.21 / 100)^2 + 1))
    }),
  ...
  Cojutti2022 = list(
    ...
    covariates = c("eGFR"),
    omega = lotri::lotri({ # reported as CV (final model)
      ETA_CL + ETA_V1 + ETA_V2 + ETA_Q ~
        c(log((26.44 / 100) ^ 2 + 1),
          0, log((16.10 / 100) ^ 2 + 1),
          0, 0, log((37.15 / 100) ^ 2 + 1),
          0, 0, 0, log((50.90 / 100) ^ 2 + 1))
    }),
  ...
  Carrothers2020 = list(
    ...
    # paper reports sigma^2 proportional = 0.0362 (variance) -> SD = sqrt(0.0362) ~ 0.19
    sigma = c(additive_a = 0, proportional_b = 0.1903)
  ),
  ...
  Lodise2026 = list(
    ...
    sigma = c(additive_a = 0, proportional_b = 0.127),  # Table 6 final model: proportional RUV for total (SD)
    auc_is_free = TRUE
  ),
    ...
    omega = lotri::lotri({ # reported as CV
      ETA_CL + ETA_V1 + ETA_V2 + ETA_Q ~
        c(
          log((22 / 100)^2 + 1),
          0, log((17.3 / 100)^2 + 1),
          0, 0, log((30.1 / 100)^2 + 1),
          0, 0, 0, log((55.9 / 100)^2 + 1)
        )
    }),
    sigma = c(additive_a = 0, proportional_b = 0.144)
  ),
  ...
  Benavent2025 = list(
    ...
    omega = lotri::lotri({ # reported as omega SD: CL 0.290, V 0.200; stored here as omega^2
      ETA_CL + ETA_V1 ~
        c(
          0.290^2,
          0, 0.200^2
