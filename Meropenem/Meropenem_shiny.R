...
model_by_type <- list(
  ...
  "Acute-on-Chronic Liver Failure (ACLF)" = c("Grensemann_2020"),
  # CNS infection: meningitis / ventriculitis — CSF IL-6 covariate
  "CNS Infection" = c("Wicha_2024")
)

# Mapping: which covariate names require which UI input group
# Used by the dynamic special-covariate renderUI
cov_to_group <- list(
  crrt     = c("CRRT", "Qfd"),
  rrt      = c("RRT"),
  cvvh     = c("CVVH", "CL_CVVH"),
          ...
          box(width = 12, solidHeader = TRUE, status = "primary",
            ...
            fluidRow(column(6,
              selectInput("patient_type", NULL,
                          ...
                          width    = "100%"),
              helpText("Selects the most appropriate default models. ",
                       "You can override the selection in the Model Specification tab.")
            )),

            # Special covariates — rendered dynamically based on selected models
            uiOutput("special_covs_ui"),
...
server <- function(input, output, session) {
  ...
  # ── Which special covariates are required by the currently selected models ──
  needed_special_covs <- reactive({
    sel <- input$model_selection %||%
             model_by_type[[input$patient_type %||% "General ICU"]]
    models_sel <- meropenem_models[names(meropenem_models) %in% sel]
    unique(unlist(lapply(models_sel, function(m) m$covariates %||% character(0))))
  })

  # ── Dynamic special-covariate UI ─────────────────────────────────────────────
  output$special_covs_ui <- renderUI({
    covs <- needed_special_covs()
    needs <- function(grp) any(cov_to_group[[grp]] %in% covs)

    sections <- list()

    if (needs("sepsis") || needs("shock") || needs("aclf")) {
    ...
    if (needs("rrt") && !needs("crrt")) {
      sections$rrt <- tagList(
        h5("Renal replacement therapy"),
        fluidRow(
          column(3, checkboxInput("flag_rrt", "Patient on any RRT", FALSE))
        )
      )
    }

    if (needs("cvvh")) {
      sections$cvvh <- tagList(
        h5("CVVH parameters"),
        fluidRow(
          column(3, checkboxInput("flag_cvvh", "CVVH active",           FALSE)),
          column(3, numericInput("cvvh_qf",    "Ultrafiltration Qf (L/h)", 2)),
          column(3, numericInput("cvvh_sc",    "Sieving coefficient Sc",   0.9)),
          column(3, numericInput("cvvh_cf",    "Correction factor CF",     1))
        )
      )
    }

    if (needs("burns")) {
      sections$burns <- tagList(
        h5("Burns"),
        fluidRow(
          column(3, checkboxInput("flag_burn", "Burns patient", FALSE)),
          column(3, numericInput("tbsa", "TBSA (%)", 30))
        )
      )
    }

    if (needs("ecmo")) {
      sections$ecmo <- tagList(
        h5("ECMO"),
        fluidRow(column(4, numericInput("lpm", "ECMO flow rate (L/min)", 4)))
      )
    }

    if (needs("csf")) {
      sections$csf <- tagList(
        h5("CNS infection"),
        fluidRow(column(4, numericInput("il6_csf", "CSF IL-6 (ng/L)", 500)))
      )
    }

    if (needs("diur")) {
      sections$diur <- tagList(
        h5("Renal function"),
        fluidRow(column(4, numericInput("diur", "Urine output (mL/24h)", 1000)))
      )
    }
    ...
    if (needs("comorbid")) {
      sections$comorbid <- tagList(
        h5("Lab values / comorbidities"),
        fluidRow(
          column(3, numericInput("wbc",    "WBC (×10⁹/L)",           10)),
          column(3, checkboxInput("flag_hta", "Hypertension",          FALSE)),
          column(3, checkboxInput("flag_van", "Concomitant vancomycin",FALSE)),
          column(3, checkboxInput("flag_col", "Concomitant colistin",  FALSE))
        )
      )
    }

    if (length(sections) == 0) return(NULL)
    tagList(hr(), h4("Additional covariates"), do.call(tagList, sections))
  })
  ...
  patient_data <- reactive({
    ...
    # CL_CVVH = Qf × Sc × CF (pre-calculated extracorporeal clearance)
    cl_cvvh_val <- if (!is.null(input$cvvh_qf) && isTRUE(input$flag_cvvh))
    ...
    cov_rows <- bind_rows(rv$weights, rv$creatinines, rv$albumins) %>%
      ...
      ungroup() %>%
      mutate(
        # Binary flags — read from explicit checkboxes (not inferred from patient type)
        CRRT    = as.integer(isTRUE(input$flag_crrt)),
        RRT     = as.integer(isTRUE(input$flag_crrt) | isTRUE(input$flag_rrt)),
        CVVH    = as.integer(isTRUE(input$flag_cvvh)),
        ACLF    = as.integer(isTRUE(input$flag_aclf)),
